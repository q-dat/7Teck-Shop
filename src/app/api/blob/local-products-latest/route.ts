...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const latestBlob = blobs
      ...
      .sort((firstBlob, secondBlob) => new Date(secondBlob.uploadedAt).getTime() - new Date(firstBlob.uploadedAt).getTime())[0];
    ...
    return NextResponse.json({
      ...
      url: latestBlob.url,
      downloadUrl: latestBlob.downloadUrl,
    ...
    return NextResponse.json(
      {
        message: error instanceof Error ? error.message : 'Không thể lấy backup mới nhất',
      },
      {
        status: 400,
      }
