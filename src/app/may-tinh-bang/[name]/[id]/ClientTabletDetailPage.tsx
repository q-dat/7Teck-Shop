...
export default function ClientTabletDetailPage({ tablet }: { tablet: ITablet }) {
  ...
  useLayoutEffect(() => {
    if (tablet && tablet?.tablet_catalog_id) {
      setLoading(false);
    }
    updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible);
  }, [tablet, tablet?.tablet_thumbnail]);

  useEffect(() => {
    scrollToTopSmoothly();
    const cleanup = handleScrollButtons(scrollRef, Object.keys(tablet.tablet_catalog_id || {}).length, () =>
      updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible)
    );
    return cleanup;
  }, [tablet]);
              ...
              <div className="relative w-full">
                <div className="h-[500px] w-full overflow-hidden object-cover">
                  <Image
                    ...
                    height={500}
                    loading="lazy"
                    src={selectedImage || tablet?.tablet_img}
                    alt={tablet?.tablet_name || 'Hình ảnh'}
                ...
                <div className="w-full">
                  ...
                  {tablet?.tablet_color && (
                    <p className="space-x-1 text-gray-500">
                      <span>Màu sắc:</span>
                      <strong className="text-black">{tablet?.tablet_color}</strong>
                    </p>
                  )}
                  {tablet?.tablet_status && (
                    <p className="space-x-1 text-gray-500">
                      <span>Tình trạng:</span>
                      <strong className="text-black">{tablet?.tablet_status}</strong>
                    </p>
                  )}
                  {tablet?.tablet_des && (
                    <p className="text-lg text-blue-500">
                      <span>{tablet?.tablet_des}</span>
                    </p>
                  )}
                </div>
                  ...
                  {tabletFieldMap.map((group) => (
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter((field) => {
                            const groupKey = group.group as keyof ITablet['tablet_catalog_id'];
                            const fieldKey = field.field;
                            const groupValue = tablet?.tablet_catalog_id?.[groupKey] as Record<string, string>;
              ...
              {activeTab === 'details' && (
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: tablet?.tablet_catalog_id?.t_cat_content ?? '',
                  }}
                ></p>
              )}
            </div>
