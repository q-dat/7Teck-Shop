...
export default function ClientTabletFC({ tablets }: ClientTabletFCProps) {
  ...
  return (
    <div className={`p-0 xl:px-desktop-padding`}>
      ...
      <div role="region" aria-label="Danh sách giảm giá mạnh" className="flex w-full flex-col items-start justify-center px-2 xl:rounded-t-lg">
        <h1 className="py-2 text-2xl font-semibold">{loading ? <>Đang tải...</> : sortedTablets.length === 0 ? <></> : <>iPad - Giảm giá mạnh</>}</h1>
      </div>
          ...
          {loading ? (
            ...
            sortedTablets.map((tablet) => {
              ...
              return (
                ...
                >
                  <Link aria-label="Xem chi tiết sản phẩm khi ấn vào hình ảnh" href={`/may-tinh-bang/${tabletUrl}/${tablet?._id}`}>
      ...
      {/* See more */}
      <Link href="/may-tinh-bang" aria-label=" Xem thêm sản phẩm iPad">
        <>
          {loading ? (
            <>Đang tải...</>
          ) : sortedTablets.length === 0 ? (
