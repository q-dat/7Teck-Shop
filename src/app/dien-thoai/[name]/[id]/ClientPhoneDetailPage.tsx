...
export default function ClientPhoneDetailPage({ phone }: { phone: IPhone }) {
  ...
  useLayoutEffect(() => {
    if (phone && phone?.phone_catalog_id) {
      setLoading(false);
    }
  ...
  useEffect(() => {
    scrollToTopSmoothly();
    const cleanup = handleScrollButtons(scrollRef, Object.keys(phone.phone_catalog_id || {}).length, () =>
      updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible)
    );
                  ...
                  {phoneFieldMap.map((group) => (
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter((field) => {
                            const groupKey = group.group as keyof IPhone['phone_catalog_id'];
                            const fieldKey = field.field;
                            const groupValue = phone?.phone_catalog_id?.[groupKey] as Record<string, string>;
