...
export default function ClientWindowsDetailPage({ win }: { win: IWindows }) {
  ...
  useEffect(() => {
    if (win && win?.windows_catalog_id) {
      setLoading(false);
    }
    scrollToTopSmoothly();
    const cleanup = handleScrollButtons(scrollRef, Object.keys(win.windows_catalog_id || {}).length, () =>
      updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible)
    );
    return cleanup;
  }, [win]);
              ...
              <div className="relative rounded-md p-1">
                ...
                >
                  {win?.windows_thumbnail && Array.isArray(win?.windows_thumbnail) ? (
                    win?.windows_thumbnail.map((thumb: string, index: number) => (
                      <Image
                        width={500}
                        height={500}
                        loading="lazy"
                ...
                <div className="w-full">
                  ...
                  {win?.windows_color && (
                    <p className="space-x-1 text-gray-500">
                      <span>Màu sắc:</span>
                      <strong className="text-black">{win?.windows_color}</strong>
                    </p>
                  )}
                  {win?.windows_status && (
                    <p className="space-x-1 text-gray-500">
                      <span>Tình trạng:</span>
                      <strong className="text-black">{win?.windows_status}</strong>
                    </p>
                  )}
                  {win?.windows_des && (
                  ...
                  {windowsFieldMap.map((group) => (
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter((field) => {
                            const groupKey = group.group as keyof IWindows['windows_catalog_id'];
                            const fieldKey = field.field;
                            const groupValue = win?.windows_catalog_id?.[groupKey] as Record<string, string>;
              ...
              {activeTab === 'details' && (
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: win?.windows_catalog_id?.w_cat_content ?? '',
                  }}
                ></p>
              )}
            </div>
