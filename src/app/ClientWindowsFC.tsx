...
export default function ClientWindowsFC({ windows }: ClientWindowsFCProps) {
  ...
  return (
    <div className={`p-0 xl:px-desktop-padding`}>
      ...
      <div role="region" aria-label="Danh sách giảm giá mạnh" className="flex w-full flex-col items-start justify-center px-2 xl:rounded-t-lg">
        <h1 className="py-2 text-2xl font-semibold">{loading ? <>Đang tải</> : sortedWindows.length === 0 ? <></> : <>Windows - Giảm giá mạnh</>}</h1>
      </div>
          ...
          {loading ? (
            ...
            sortedWindows.map((win) => {
              ...
              return (
                ...
                >
                  <Link aria-label="Xem chi tiết sản phẩm khi ấn vào hình ảnh" href={`/windows/${tabletUrl}/${win?._id}`}>
                  ...
                  <div className="flex h-full w-full flex-col items-start justify-between p-1">
                    <Link
                      ...
                      href={`/windows/${tabletUrl}/${win?._id}`}
                    >
                      <p className="xl:group-hover:text-secondary">Điện Thoại {win.windows_name}</p>
                    </Link>
                  ...
                  {win?.windows_status && (
                    <div className="absolute -left-[3px] top-0 z-20">
                      ...
                      <p className="absolute top-[1px] w-full pl-2 text-xs text-white">{win?.windows_status}</p>
                    </div>
                  )}
                </div>
              );
      ...
      {/* See more */}
      <Link href="/windows" aria-label="Xem thêm sản phẩm laptop Windows">
        <>
          {loading ? (
            <>Đang tải...</>
          ) : sortedWindows.length === 0 ? (
