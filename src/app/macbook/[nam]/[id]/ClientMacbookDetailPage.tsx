...
export default function ClientMacbookDetailPage({ mac }: { mac: IMacbook }) {
  ...
  useEffect(() => {
    scrollToTopSmoothly();
    const cleanup = handleScrollButtons(scrollRef, Object.keys(mac.macbook_catalog_id || {}).length, () =>
      updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible)
    );
    return cleanup;
  }, [mac]);
                  ...
                  {macbookFieldMap.map((group) => (
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter((field) => {
                            const groupKey = group.group as keyof IMacbook['macbook_catalog_id'];
                            const fieldKey = field.field;
                            const groupValue = mac?.macbook_catalog_id?.[groupKey] as Record<string, string>;
              ...
              {activeTab === 'details' && (
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: mac?.macbook_catalog_id?.m_cat_content ?? '',
                  }}
                ></p>
              )}
            </div>
